...
class RapidAPIRateLimiter:
    ...
    def wait_before_request(self):
        ...
        if len(self.request_times) >= self.max_requests_per_window:
            ...
            wait_seconds = (
                oldest_request
                + timedelta(seconds=self.window_seconds + self.safety_margin)
                - now
            ).total_seconds()

            if wait_seconds > 0:
                logger.warning(
                    f"Rate limit window full. Waiting {wait_seconds:.2f}s..."
                )
        ...
        if remaining is not None:
            ...
            if remaining <= 0:
                ...
                if not self.wait_on_quota_exhausted:
                    logger.error(
                        f"Daily quota exhausted. Recovery time: {self.quota_reset_at}"
                    )
                    raise DailyQuotaExhausted(self.quota_reset_at)

                wait_seconds = reset_seconds + self.safety_margin
                logger.warning(
                    f"RapidAPI quota exhausted. Waiting {wait_seconds}s until {self.quota_reset_at}..."
                )
                time.sleep(wait_seconds)
